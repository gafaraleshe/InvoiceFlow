<!--
  ACTIVE DIRECTION: Hermite Flow / Hermite Labs uses the Composio-inspired
  design language — near-black canvas with an electric-blue voltage (#0007cd /
  #1a26ff), spotlight glows, and terminal-style panels. The full reference is in
  ./composio/DESIGN.md (generated by `npx getdesign@latest add composio`) and is
  the source of truth for new UI. The Linear analysis below is kept for its
  layout/rhythm patterns; use the Composio palette + accent over its colors.
-->
---
version: alpha
